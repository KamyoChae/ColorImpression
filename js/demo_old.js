...
function ready() {
    ...
    this.reading = setInterval(function () {
        if (read == 0) {
            // 此处不会进入循环，原因未知，有待后人查找
            console.log('555555555555555555555555666666666666666666666666666666666666666')
...
function setCover() {
    ...
    this.cover = setTimeout(function () {
        ...
        /*
        * 类数组转换成数组的方法 var a = Array.prototype.slice.call(getChilds); 
           另一个遍历方法 for(var prop in getChilds){
             // if(getChilds.hasOwnProperty(prop)){
                    if(!getChilds[prop]){
                        getChilds[prop].style.display = 'block'
                }
            }
        */
